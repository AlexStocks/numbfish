 ...
 * @return =0 成功 <0 失败
 */
int32_t update_rdata_by_zk_service_json_data(const char *name, const char *value, int32_t value_len, uint64_t mtime)
{
    int32_t ret;
    ...
    rdata = get_local_rdata(name);
    if (rdata != NULL) {
        update_rdata_by_zk_service_nodes(rdata, servers, mtime);
        free(servers);
        return 0;
    }

    /* 如果本地没有该业务路由数据，需要重新创建 */
    ret = add_rdata(name, servers, mtime);
    if (ret < 0) {
...
/**
 * @brief  获取业务配置信息回调函数
 * @info   需要区分新业务请求和NLB_EVENT_TYPE_GET_SERVICE_NODES事件两种情况
 * @return =0 成功 <0 失败
 */
static void nameservice_aget_completion(int32_t rc, const char *value, int32_t value_len,
                                 const struct Stat *stat, const void *ctx)
{
    int32_t ret;
    struct agent_local_rdata *rdata;
    ...
    /* 没有该业务 */
    if (rc == ZNONODE) {
        NLOG_DEBUG("get service (%s) route config failed, [%s]", (char *)ctx, zerror(rc));
        goto ERR_RET;
    }

    /* zookeeper自身错误 */
    if (rc != ZOK) {
        NLOG_ERROR("get service (%s) route config failed, [%s]", (char *)ctx, zerror(rc));
        goto ERR_RET;
    }

    /* 业务配置为空 */
    if (value_len == 0) {
        NLOG_ERROR("service (%s) route config is null", (char *)ctx);
        goto ERR_RET;
    }

    rdata = get_local_rdata((char *)ctx);
    if (rdata != NULL) {
        /* 如果和本地的路由数据时间戳一致，不更新 */
        if (rdata->route_meta->mtime == (uint64_t)stat->mtime) {
            free((void *)ctx);
    ...
    NLOG_INFO("receive service (%s) config", (char *)ctx);

    /* 处理配置 */
    ret = update_rdata_by_zk_service_json_data((const char *)ctx, value, value_len, (uint64_t)stat->mtime);
    if (ret < 0) {
    ...
    /* 业务配置加载成功，需要处理路由请求任务 */
    process_route_task((char *)ctx);
    free((void *)ctx);
    return;

ERR_RET:
    delete_route_task((char *)ctx);  // 这里没有判断是否首次加载失败，会有重复调用的问题
    free((void *)ctx);
    return;
}
...
/**
 * @brief  获取业务配置信息
 * @info   只有新业务请求和NLB_EVENT_TYPE_GET_SERVICE_NODES事件会调用该函数
 * @return =0 成功 <0 失败
 */
int32_t get_service_nodes(const char *name)
{
    int32_t ret, result;
    char  path[NLB_PATH_MAX_LEN];
    char *ctx = NULL;
