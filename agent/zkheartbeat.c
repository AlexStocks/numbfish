 ...
 * @brief 创建心跳节点回调函数
 */
static void heartbeat_create_complete(int32_t rc, const char *name, const void *data)
{
...
int32_t create_heartbeat_node(uint32_t ip)
{
    ...
    ret = zoo_acreate(get_zk_instance(), path, NULL, 0, &ZOO_OPEN_ACL_UNSAFE, flags,
                      heartbeat_create_complete, (void *)(long)ip);
    if (ret != ZOK) {
 ...
 */
static void heartbeat_exist_complete(int32_t rc, const struct Stat *stat, const void *data)
{
    uint32_t ip = (uint32_t)(long)data;
    ...
    NLOG_ERROR("heartbeat_complete failed, [%s]", zerror(rc));
...
int32_t set_node_watcher(uint32_t ip)
{
    ...
    if (!is_node_watching(ip) && zk_connected()) {
        ...
        ret = zoo_awexists(get_zk_instance(), path, heartbeat_exist_watcher, (void *)(long)ip,
                           heartbeat_exist_complete, (void *)(long)ip);
        if (ret != ZOK) {
